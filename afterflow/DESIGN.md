...
The navigation is a transparent, borderless 64px overlay that belongs to the opening image and leaves the viewport after the hero on wide screens. The supplied mark renders white without a tile so it belongs to the image plane; 16px links use compact spacing and reveal a one-pixel cyan underline on hover or focus. Below 900px, the compact header stays available on the long page and gains a near-black blurred field after scrolling only while the menu is closed. Its 44px circular trigger opens a square-edged, full-viewport decision field that continues behind the header, pairs a darkened incumbent world image with grain, and presents four large hairline-separated links plus the signal action. The field resolves in one top-down structural sweep, followed by a restrained blur-and-rise link sequence; closing mirrors that composition more quickly with a bottom-to-top sweep, reverse-order link departure, and a slight world-image recession. Reduced-motion users receive the complete open or closed state immediately.
